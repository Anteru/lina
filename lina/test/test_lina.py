...
from .. import (
    TemplateRepository, Template, InvalidFormatter,
    InvalidToken, InvalidWhitespaceToken, TemplateException, InvalidBlock
)
import pytest
...
def testValueFormatterOnBlockFails():
    # This must fail because blocks can only use block formatters
    with pytest.raises(InvalidFormatter):
        template = Template('{{#block:escape-string}}{{item}}{{/block}}')
        template.RenderSimple(block=[{'item': '0'}])


def testBlockFormatterOnValueFails():
    with pytest.raises(InvalidFormatter):
        template = Template('{{item:l-s=NEWLINE}}')
        template.RenderSimple(item='0')
...
def testExpandCompoundThrowsIfNoSuchFieldNone():
    ...
    with pytest.raises(TemplateException):
        template.RenderSimple(items=[{'item': None}])


def testExpandCompoundThrowsIfNoSuchFieldEmptyDict():
    template = Template('{{#items}}This is {{item.field}}{{/items}}')
    with pytest.raises(TemplateException):
        template.RenderSimple(items=[{'item': {}}])
